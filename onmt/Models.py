...
class RNNTrigramsEncoder(RNNEncoder):
    ...
    def forward(self, src, lengths=None, encoder_state=None, batch_size=64):
        "See :obj:`EncoderBase.forward()`"
        self._check_args(src, lengths, encoder_state)
...
class RNNTrigramDecoderBase(nn.Module):
    """
    Base recurrent attention-based decoder class.
    Specifies the interface used by different decoder types
    and required by :obj:`onmt.Models.NMTModel`.
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
                 ...
                 copy_attn=False, dropout=0.0, embeddings=None,
                 reuse_copy_attn=False):
        super(RNNTrigramDecoderBase, self).__init__()
...
class InputFeedTrigramRNNDecoder(RNNTrigramDecoderBase):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
                 ...
                 copy_attn=False, dropout=0.0, embeddings=None,
                 reuse_copy_attn=False):
        super(InputFeedTrigramRNNDecoder, self)\
        ...
        self.rnn_trigram, _ = \
            rnn_factory(rnn_type,
                        ...
                        dropout=dropout,
                        bidirectional=True)
        self.combine_states = nn.Linear(2*hidden_size, hidden_size)
    ...
    def _run_forward_pass(self, tgt, memory_bank, state, memory_lengths=None):
        ...
        words_memory_bank, trigram_final = self.rnn_trigram(emb)
        back_state, forw_state = words_memory_bank[0, :, self.rnn_size:], words_memory_bank[-1, :, :self.rnn_size]
        word_embs = self.combine_states(torch.cat([back_state, forw_state], dim=-1))
        word_embs = word_embs.view(batch_seqlen // self.batch_size, self.batch_size, emb_dim)
...
class NMTTrigramModel(nn.Module):
    """
    Core trainable object in OpenNMT. Implements a trainable interface
    for a simple, generic encoder + decoder model.
    ...
    def __init__(self, encoder, decoder, multigpu=False):
        self.multigpu = multigpu
        super(NMTTrigramModel, self).__init__()
        self.encoder = encoder
        self.decoder = decoder
        self.decoder.bidirectional_encoder = True

    def forward(self, src, tgt, lengths, dec_state=None, batch_size=64):
        """Forward propagate a `src` and `tgt` pair for training.
        Possible initialized with a beginning decoder state.
