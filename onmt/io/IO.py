...
from .Wordbatch import WordIterator
...
def make_features(batch, side, data_type='text'):
    ...
    if data_type in ['text', 'text-trigram']:
...
def convert_into_trigrams(inputtuple):
    sen = []
    for w in inputtuple:
        l = int(w.__len__())
        for i in range(0, l):
    ...
    for path in train_dataset_files:
        ...
        for ex in dataset.examples:
            for k in fields:
                ...
                if data_type == 'text-trigram' and val is not None and k in ["src"]:
                    val = convert_into_trigrams(val)
...
class OrderedTrigramIterator(WordIterator):
    def create_batches(self):
        if self.train:
            self.batches = torchtext.data.pool(
                self.data(), self.batch_size,
